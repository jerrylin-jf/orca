...
// Why: we generate the image ourselves via nativeImage.toPNG(), so the accepted
...
export function isOpenInAppIconId(value: unknown): value is OpenInAppIconId {
...
export function isOpenInAppIconImageSrc(value: unknown): value is string {
  ...
  return payload.length > 0 && BASE64_PATTERN.test(payload)
}

export function openInAppIconImage(src: string, label?: string): OpenInAppIcon {
  const trimmedLabel = label?.trim()
  return trimmedLabel ? { type: 'image', src, label: trimmedLabel } : { type: 'image', src }
}
...
export function normalizeOpenInAppIcon(value: unknown): OpenInAppIcon | null {
  ...
  const candidate = value as { type?: unknown; id?: unknown; src?: unknown; label?: unknown }
